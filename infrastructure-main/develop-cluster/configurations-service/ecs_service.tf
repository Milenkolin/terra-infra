resource "aws_ecs_task_definition" "configurations" {
  family                   = "${data.terraform_remote_state.develop_cluster.outputs.environment}-configurations-service"
  ...
  [
    ...
    "logConfiguration": {
      ...
      "secretOptions": [],
      "options": {
        "awslogs-group": "develop-configurations-service",
    ...
    "systemControls": [],
    "privileged": null,
    "name": "develop-configurations-service"
  }
  ]
EOF
...
resource "aws_ecs_service" "configurations" {
  name                              = "${data.terraform_remote_state.develop_cluster.outputs.environment}-configurations-service-Service-Sc6MMMQMckpC"
  ...
  load_balancer {
    ...
    container_name   = "${data.terraform_remote_state.develop_cluster.outputs.environment}-configurations-service"
    container_port   = 8333
  }
