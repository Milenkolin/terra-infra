resource "aws_lb" "configurations_lb_JSY" {
  name                   = "devel-LoadB-JSYRYJ94MG49"
  internal               = true
  load_balancer_type     = "application"
  security_groups        = [var.sg_for_lb_JSY]
  subnets            = [
  ...
  tags = {
    "Name" = "develop-cluster-internal-lb"
  }
...
resource "aws_lb_listener" "configurations_https" {
  load_balancer_arn = aws_lb.configurations_lb_JSY.arn
  port              = 443
  protocol          = "HTTPS"
...
resource "aws_lb_listener_rule" "configurations" {
  ...
  condition {
    host_header {
      values = ["configurations.develop.stockperks.internal", "api-develop.stockperks.com"]
    }
  }
  tags     = {}
  tags_all = {}
