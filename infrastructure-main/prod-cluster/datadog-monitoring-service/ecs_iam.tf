resource "aws_iam_role" "ecs-datadog-role" {
  name               = "${data.terraform_remote_state.prod_cluster.outputs.environment}-ecs-datadog-role"
...
resource "aws_iam_policy" "ecs-datadog-role-policy" {
  name   = "${data.terraform_remote_state.prod_cluster.outputs.environment}-datadog-agent-ecs-iam-role-policy"
