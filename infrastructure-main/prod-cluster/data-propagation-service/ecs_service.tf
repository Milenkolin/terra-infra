...
resource "aws_security_group" "lb_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = var.infra_vpc_cidr_blocks
  }
...
resource "aws_appautoscaling_target" "data_propagation_service_scaling" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/${data.terraform_remote_state.prod_cluster.outputs.cluster_name}/${local.env}-data-propagation-service"
