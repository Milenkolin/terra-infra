...
resource "aws_db_subnet_group" "data_propagation_service" {
  name = "${local.env}-data-propagation-service-rds"
  subnet_ids = [
    data.terraform_remote_state.prod_cluster.outputs.public_subnet_id_us-east-1a,
    data.terraform_remote_state.prod_cluster.outputs.public_subnet_id_us-east-1b
  ]
