resource "aws_ecs_task_definition" "notifications" {
  family                   = "${data.terraform_remote_state.prod_cluster.outputs.environment}-notifications"
  task_role_arn            = "arn:aws:iam::979370138172:role/prod-notifications-service-role-IamRole-6PLFWM9QQ2WX"
  ...
  [
    {
      ...
      "logConfiguration": {
        ...
        "secretOptions": [],
        "options": {
          "awslogs-group": "${data.terraform_remote_state.prod_cluster.outputs.environment}-notifications",
    ...
    "systemControls": [],
    "privileged": null,
    "name": "${data.terraform_remote_state.prod_cluster.outputs.environment}-notifications"
    }
  ]
EOF
...
resource "aws_ecs_service" "notifications" {
  name                       = "${data.terraform_remote_state.prod_cluster.outputs.environment}-notifications-service-Service-eHm3JcLxvu4u"
